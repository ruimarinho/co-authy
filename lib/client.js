...
var phoneNumberFormat = require('./formatters/phone-number-formatter');
var sf = require('sf');
 ...
 *                      ISO 3166-1 alpha-3 code (e.g. PTR) or a calling code
 *                      (e.g. 351). Defaults to US (1) if omitted.
 * @return {Integer} The `authyId` of the registered user.
 * @api public
 */

AuthyClient.prototype.registerUser = function *(email, cellphone, code) {
  var parsed = phoneNumberFormat.parseRawInput(cellphone, code);
  ...
  try {
    data.cellphone = phoneNumberFormat.e164(data.cellphone, parsed.countryCode).replace('+' + parsed.countryCallingCode, '');
  } catch (e) {}
